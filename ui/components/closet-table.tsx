...
export function ClosetTable(props: {
  ...
  useEffect(() => {
    switch (sortBy) {
      ...
      case "rating":
        setOutfitItems([
          ...outfitItems.sort((a, b) => (a.rating < b.rating ? -1 : 1)),
        ]);
        break;
      case "rating-reverse":
        setOutfitItems([
  ...
  return (
    <>
      {itemsSelected && itemsSelected.length == outfitItems.length ? (
      ...
      itemsSelected && itemsSelected.length > 1 ? (
        ...
        itemsSelected.length > 0 && (
          <ItemCostPerWear
            ...
            count={
              outfitItemToIds.get(itemsSelected[0])
                ? outfitItemToIds.get(itemsSelected[0])?.length
                : 1
            }
          />
        )
      ...
      {
        <h6 className="mb-2">
          Select items from the closet below to update graphs and see outfits
          that contain them.
        </h6>
      }
        ...
        <table className="w-full text-xs md:text-sm text-left overflow-x-scroll">
          <thead className="text-xs uppercase bg-background sticky top-0">
            <tr>
              ...
              {props.includeEdit && (
                <th scope="col" className="p-2">
                  <div className="flex items-center gap-1 text-xs">edit</div>
                </th>
              )}
              <th scope="col" className="p-2">
                ...
                <td className="p-2">
                  <div className="flex gap-1">
                    <input
                      type="checkbox"
                      className={
                        handleItemSelection == null ? "cursor-not-allowed" : ""
                      }
                      onChange={() => handleItemSelection(item.id)}
                      checked={
                        itemsSelected ? itemsSelected.includes(item.id) : false
                      }
                    ></input>
      ...
      {!props.onlyTable && (
        <>
          <div className="my-4 p-1 bg-primary w-fit rounded text-white">
