...
export function SubmitOutfit(props: {
  clientServer: string;
  cookie: string;
  handleClose: any;
  closetName: string;
  ...
  const [submissionValidationErr, setSubmissionValidationErr] = useState<
    string | null
  >(null);
  const [submissionStatus, setSubmissionStatus] = useState<string | null>(null);
  const [outfitsSelected, setOutfitsSelected] = useState<
    BusinessOutfitSelected[] | null
  >(null);
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (!outfitsSelected || outfitsSelected.length == 0) {
      setSubmissionValidationErr(
        "Please select at least 1 outfit and 1 outfit item to submit to the closet."
      );
      return;
    }
    ...
    if (missing) {
      setSubmissionValidationErr(
        "Please select at least 1 item in each outfit to submit to the closet."
      );
      return;
    }

    const resp = await PostBusinessOutfits(
      props.clientServer,
      props.cookie,
      outfitsSelected,
      props.closetName
    ...
    <Modal handleClose={props.handleClose} wideScreen={true} fullHeight={true}>
      <>
        <h1 className="mb-8 capitalize">
          Submit an Outfit to {props.closetName}&apos; Closet
        </h1>

        {err && (
          <div className="bg-red-700 w-fit p-2 text-white mb-4">
            Error: {err}
          </div>
        )}

        {submissionStatus == "success" ? (
          <h2>Your outfits have been successfully submitted.</h2>
        ) : (
          submissionStatus && (
            <>
              <h2 className="text-primary">Error submitting your outfits</h2>
              <div>
                Please try again by refreshing the page. If the issue persists,
                please email rateyourstyle@gmail.com.
              </div>
            </>
          )
        )}
        ...
        {!err && !submissionStatus && (
          <>
            {!userOutfits || userOutfits.length == 0 ? (
              <div>
                No outfits from your closet to choose from.{" "}
                <Link href="/post-outfit" passHref={true}>
                  Post an Outfit
                </Link>{" "}
                to start populating your closet.
              </div>
            ) : (
              <div>
                <div className="flex gap-4 flex-wrap">
                  {userOutfits.map((outfit) => {
                    let selected =
                      outfitsSelected &&
                      outfitsSelected.filter((e) => e.outfit_id === outfit.id)
                        .length > 0;
                    ...
                    if (selected) {
                      selectedItems =
                        outfitsSelected &&
                        outfitsSelected.filter(
                          (e) => e.outfit_id === outfit.id
                        )[0].item_ids;
                    }
                    ...
                    return (
                      <div
                        className={`w-48 shadow-md break-words  ${
                          selected ? "border-4 border-primary" : "border-none"
                        }`}
                        key={outfit.id}
                      >
                        <img
                              ...
                              return (
                                ...
                                >
                                  <input
                                    ...
                                    type="checkbox"
                                    onChange={() =>
                                      handleOutfitItemClick(outfit.id, i.id)
                                    }
                                    checked={
                                      selectedItems &&
                                      selectedItems.filter((e) => e === i.id)
                                        .length > 0
                                        ? true
                                        : false
                                    }
                ...
                {submissionValidationErr && (
                  <div className="bg-red-700 w-fit p-2 text-white mt-4">
                    Required: {submissionValidationErr}
                  </div>
                )}
  ...
  return (
    <div>
      <h6 className="capitalize">
        {props.count}.{" "}
        {props.i.link ? (
          <a href={props.i.link} target="_blank" className="">
            {props.i.color} {props.i.description}{" "}
          </a>
        ) : (
